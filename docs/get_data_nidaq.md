...
Using GUI to acquire data is really straightforward and requires only
...
The user is now able to select the desired device, channel, terminal configuration
(Differential, RSE and NRSE), sample period and session duration. Also,
the user will define if the data will be plotted and saved or not, as well as the path to
save data.
...
Then, instantiate a class with the defined parameters and get the data
...
**NOTE**: data will be saved on the desktop, by default. To change the path, the user can define "g.path = Desired path"
...
Data will also be saved as depicted as shown below:
